...
static Value array_iterator_next(Context *ctx, Value this_val, int, const Value *) {
  ...
  if (!iter) return Value::undefined_();

  uint32_t idx = 0;
  Value idx_v  = iter->get_own(rt->intern("_idx"));
  if (idx_v.is_int32()) idx = static_cast<uint32_t>(idx_v.as_int32());

  Value arr_v = iter->get_own(rt->intern("_arr"));
  if (!arr_v.is_object()) goto done;
  {
    auto *o = arr_v.as<Object>();
    if (o->class_id != ClassID::array) goto done;
    auto *arr = static_cast<ArrayObject *>(o);
    if (idx >= arr->elements.size()) goto done;
...
void init_array_prototype(Context *ctx) {
  ...
  proto->set_own(rt, static_cast<Atom>(AtomEnum::Symbol_iterator), si_fn);
  proto->set_own(rt, rt->intern("Symbol.iterator"), si_fn); // debug: string key too
}
