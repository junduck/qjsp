...
struct MallocState { size_t malloc_count; size_t malloc_size; size_t malloc_limit; void* opaque; };
...
struct SharedArrayBufferFunctions { SABAllocFunc* sab_alloc; SABFreeFunc* sab_free; SABDupFunc* sab_dup; void* sab_opaque; };
...
struct Runtime {
  ...
  union { ModuleLoaderFunc* module_loader_func = nullptr; ModuleLoaderFunc2* module_loader_func2; } u;
  ...
  struct ShapeKey {
    Shape* from; Atom atom; int flags;
  ...
  void remove_gc_object(GCObjectHeader* obj) {
    ...
    if (it != gc_objects.end()) { std::swap(*it, gc_objects.back()); gc_objects.pop_back(); }
  }
