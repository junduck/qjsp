...
struct Runtime {
  ...
  // Atom table: index → String*  (index 0 = null)
  ...
  // Shape factory + cache. Shapes live until Runtime::destroy().
  struct ShapeKey {
    Shape *from;
    Atom atom;
    int flags;
  ...
  struct ShapeKeyHash {
    size_t operator()(const ShapeKey &k) const {
      return std::hash<uintptr_t>{}(reinterpret_cast<uintptr_t>(k.from)) ^ static_cast<size_t>(k.atom) ^ static_cast<size_t>(k.flags);
    }
  };
