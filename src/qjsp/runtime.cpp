...
Runtime* Runtime::create() {
  auto* rt = new Runtime();
  if (!rt->init_atoms()) goto fail;
  if (!rt->init_class_table()) goto fail;
  return rt;
fail:
  rt->destroy();
  return nullptr;
...
void Runtime::destroy() {
  for (auto* s : atom_table) { if (s) s->free(); }
  for (auto& [_, s] : shape_cache) delete s;
  delete this;
}
...
String* Runtime::atom_to_string(Atom a) const {
  if (a == kAtomNull || a >= static_cast<Atom>(atom_table.size())) return nullptr;
  return atom_table[a];
}
...
Shape* Runtime::add_shape(Shape* from, Atom atom, int flags) {
  ...
  auto* s = new Shape();
  if (from) s->entries = from->entries;
...
static void mark_object(GCObjectHeader& hdr, std::vector<GCObjectHeader*>& worklist) {
  ...
  case GCObjType::js_object:  static_cast<Object&>(hdr).gc_mark(worklist); break;
  case GCObjType::js_context: static_cast<Context&>(hdr).gc_mark(worklist); break;
  default: hdr.is_marked = true; break;
  }
}

static void gc_free_object(GCObjectHeader* p) {
  switch (p->gc_obj_type) {
  case GCObjType::js_object:  delete static_cast<Object*>(p); break;
  case GCObjType::js_context: delete static_cast<Context*>(p); break;
  default: break;
  }
}
...
void Runtime::run_gc() {
  ...
  for (auto* obj : gc_objects) obj->is_marked = false;
  ...
  gc_alloc_count = 0;
  malloc_gc_threshold = malloc_gc_threshold > 0
    ? malloc_gc_threshold + (malloc_gc_threshold >> 1) : 1024;
}

void Runtime::maybe_trigger_gc(size_t) {
  if (++gc_alloc_count >= malloc_gc_threshold) run_gc();
}
